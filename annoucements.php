<?php
/*
Plugin Name: Responsive News & Announcements
Plugin URI: http://www.rupok.me/projects
Description: An announcement plugin that shows your anouncements/breaking news on top of the website. Stylish show of anouncements will attract the eye of your visitors.
Version: 1.0
...
define('RESPONSIVE_ANNOUNCEMENTS_PATH', plugin_dir_url( __FILE__ ));
...
function sap_register_announcements() {
	...
	$labels = array(
 	...
 	$args = array(
     	'labels' => $labels,
     	'singular_label' => __('Announcement', 'responsive-announcements'),
     	'public' => true,
...
function sap_backend_scripts($hook) {
	...
	wp_enqueue_style( 'datepicker-style', RESPONSIVE_ANNOUNCEMENTS_PATH . 'css/ui-lightness/jquery-ui.css');	 
	wp_enqueue_script( 'jquery-ui-datepicker', get_template_directory_uri() .'/wp-includes/functions.wp-scripts.php' ); 
    wp_enqueue_script( 'announcements', RESPONSIVE_ANNOUNCEMENTS_PATH . 'js/announcements.js', array( 'jquery' ) );
}
add_action('admin_enqueue_scripts', 'sap_backend_scripts');

function sap_frontend_scripts() {
	wp_enqueue_style( 'announcements-style', RESPONSIVE_ANNOUNCEMENTS_PATH . 'css/announcements.css');	 
    wp_enqueue_script( 'announcements', RESPONSIVE_ANNOUNCEMENTS_PATH . 'js/announcements.js', array( 'jquery' ) );
    wp_enqueue_script( 'cookies', RESPONSIVE_ANNOUNCEMENTS_PATH . 'js/jquery.cookie.js', array( 'jquery' ) );    
    wp_enqueue_script( 'cycle', RESPONSIVE_ANNOUNCEMENTS_PATH . 'js/jquery.cycle.lite.js', array( 'jquery' ) );    
}
    ...
    if($announcements) :
        ?>
            <div id="announcements" class="hidden"> 
                <div class="wrapper">
                    <a class="close" href="#" id="close"><?php _e('x', 'RESPONSIVE-announcements'); ?></a>                    
